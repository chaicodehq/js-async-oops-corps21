...
export class TempleQueue {
  ...
  enqueue(name, type) {
    // Your code here
    if (!["regular", "vip"].some((t) => t === type)) return null;
    else if (!name) return null;
    else if (this.#devotees.length >= this.#maxCapacity) return null;
    const devotee = {
      name,
      type,
  ...
  contains(name) {
    // Your code here
    const res = this.#devotees.find((devotee) => devotee.name === name);
    return !!res;
  }
  ...
  static merge(queue1, queue2) {
    // Your code here
    const queue = new TempleQueue(
      `${queue1.templeName}-${queue2.templeName}`,
      queue1.length + queue2.length + 10,
    );
    for (const devotee of queue1.toArray()) {
      queue.enqueue(devotee.name, devotee.type);
    }
